  ...
  <div class="invest-history">
    <div class="invest-editor-header">
      ...
      <div class="invest-history-titles">
        <span class="invest-editor-title">{{ t('invest.compareHistoryTitle') }}</span>
      </div>
      <button
    ...
    <div v-else-if="entries.length === 0" class="invest-empty invest-history-empty">
      <span class="material-symbols-outlined invest-empty-icon">compare_arrows</span>
      <p>{{ t('invest.compareHistoryEmpty') }}</p>
    </div>
      ...
      <div
        ...
        <button
          ...
          <div class="invest-history-row-main">
            ...
            <div class="invest-compare-history-strategies">
              <span
                v-for="(name, i) in e.strategyNames"
                :key="i"
                class="invest-compare-history-strategy-tag">
                {{ name }}
              </span>
            </div>
          </div>
          <div class="invest-history-row-metrics">
            <span class="invest-compare-history-count">
              {{ t('invest.compareHistoryCount', { n: e.results.length }) }}
            </span>
...
import type { InvestCompareHistoryEntry } from '@/types/invest';

const props = defineProps<{
  entries: InvestCompareHistoryEntry[];
  loading: boolean;
}>();
...
/* ── Compare-specific ── */
.invest-compare-history-strategies {
  display: flex;
  flex-wrap: wrap;
  gap: 4px;
}
.invest-compare-history-strategy-tag {

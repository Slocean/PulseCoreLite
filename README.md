...
# PulseCore Lite v1.8.10 <img src="src/static/img/ice_logo.webp" alt="Logo" height="28" />
...
- 内存整理：支持系统级工作集清理与定时间隔配置
- 自动更新：支持 updater 检查失败后回退 latest.json，并提供手动下载地址
- 架构分层：页面编排、窗口管理、存储仓储拆分，降低多窗口耦合
- 组件体系：统一 UiButton / UiSelect / UiDateInput / UiTimeInput / UiToast / UiDialog
- 质量门禁：ESLint + Vitest + 组件契约检查 + GitHub Actions 前端质量流程
- 主页面导航：支持 monitor/toolkit/settings 三段切换
- 工具箱集成：主页面支持内嵌工具箱，同时保留独立工具箱窗口
- 加载策略：OverlayConfigPanel 与 ToolkitEmbedded 采用异步加载
- 任务栏主题：支持 transparent / dark / light 三种主题并按语言切换文案
- 页签交互：NavTabs 支持受控模式与无障碍属性，便于状态驱动与键盘访问
- 样式兼容：引入 Autoprefixer 并使用 browserslist 约束目标浏览器
- 发布自动化：新增 release workflow，覆盖发布说明提取、构建与发布上传
- 提醒高级设置：支持图片背景、频率选择与关闭密码配置
- 偏好持久化：悬浮窗/任务栏偏好防抖写入，降低频繁 IO
- Tauri 调试：新增运行时调试日志与回调保护
...
- Improve taskbar monitor sync and context menu behavior (hide "Close Taskbar Monitor" when the main window is hidden)
...
- Memory trim: process-tree working set cleanup with configurable interval
- Auto update: fallback to `latest.json` when updater check fails, with manual download guidance
- Architecture layers: split page orchestration, window manager, and storage repository to reduce coupling
- UI system: unified UiButton / UiSelect / UiDateInput / UiTimeInput / UiToast / UiDialog
- Quality gates: ESLint + Vitest + component contract checks + GitHub Actions frontend quality pipeline
- Main navigation: switch between monitor/toolkit/settings from a unified top-level tab set
- Toolkit integration: embed Toolkit in the main page while keeping the standalone toolkit window
- Loading strategy: async-load OverlayConfigPanel and ToolkitEmbedded to improve startup responsiveness
- Taskbar themes: transparent, dark, and light modes with localized labels
- Tab interaction: NavTabs supports controlled state and accessibility attributes
- Style compatibility: Autoprefixer enabled with browserslist target ranges
- Release automation: release workflow handles notes extraction, build, and publishing
- Reminder advanced settings: image backgrounds, frequency selection, and close-password options
- Preference persistence: debounced writes for overlay/taskbar prefs to reduce IO churn
- Tauri debugging: runtime debug logs with callback guards
